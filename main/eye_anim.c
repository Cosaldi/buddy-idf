/*
 * eye_anim.c  –  DeskBuddy eye animation (LVGL v8, SSD1306 128×64)
 *
 * Akno-style: two rounded rectangles as eyes.
 * No separate pupil — expressions via height change, flat cuts, offsets.
 *
 * Eye geometry (reference, normal state)
 *   Eye W  : 45 px   Eye H  : 30 px   Corner R: 8 px
 *   Gap between eyes: 14 px
 *   Left eye centre : (32, 32)   Right eye centre: (96, 32)
 *   Eyes can shift ±8 px H, ±5 px V (look direction)
 *
 * Expressions
 *   NORMAL    – full rounded rect, centred
 *   HAPPY     – eye height reduced, bottom flat (fillrect clips lower arc)
 ...
 */
static void draw_rrect(int x, int y, int w, int h, int r,
                       lv_color_t color)
{
    if (w <= 0 || h <= 0) return;
    if (r > w / 2) r = w / 2;
...
static void draw_triangle(int x0, int y0, int x1, int y1,
                          int x2, int y2, lv_color_t color)
{
    ...
    for (int i = 0; i < 2; i++)
        for (int j = i+1; j < 3; j++)
            if (pts[j].y < pts[i].y) {
                lv_point_t t = pts[i]; pts[i] = pts[j]; pts[j] = t;
            }
    ...
    for (int y = pts[0].y; y <= pts[2].y; y++) {
        ...
        for (int e = 0; e < 3; e++) {
            ...
            if (ay == by) continue;
            if (y < LV_MIN(ay,by) || y > LV_MAX(ay,by)) continue;
            int x = ax + (bx - ax) * (y - ay) / (by - ay);
            if (x < xlo) xlo = x;
            if (x > xhi) xhi = x;
        }
        if (xhi < xlo || y < 0 || y >= DISP_H) continue;
        xlo = LV_MAX(xlo, 0); xhi = LV_MIN(xhi, DISP_W-1);
...
static void render_frame(void)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (g.expr == EYE_EXPR_HAPPY && h_scale > 0.1f) {
            ...
            int cut = eye_h / 2;
            draw_rect(ex - 1, ey + cut, EYE_W + 2, eye_h - cut + 2, black);
        }
...
{
    if (g.blink_phase == BLINK_IDLE) return;
    g.blink_step++;
...
static void look_tick(void)
{
    ...
    int step = 2;
    if (g.cur_dx < g.tgt_dx) g.cur_dx = LV_MIN(g.cur_dx + step, g.tgt_dx);
    else if (g.cur_dx > g.tgt_dx) g.cur_dx = LV_MAX(g.cur_dx - step, g.tgt_dx);
    if (g.cur_dy < g.tgt_dy) g.cur_dy = LV_MIN(g.cur_dy + step, g.tgt_dy);
    else if (g.cur_dy > g.tgt_dy) g.cur_dy = LV_MAX(g.cur_dy - step, g.tgt_dy);
}

static void idle_tick(void)
{
    if (!g.idle_enabled) return;
    uint32_t t = now_ms();
...
void eye_anim_init(lv_obj_t *parent)
{
    ...
    lv_canvas_set_buffer(g.canvas, g.cbuf, DISP_W, DISP_H,
...
{
    switch (dir) {
        case EYE_LOOK_LEFT:   g.tgt_dx = -LOOK_DX; g.tgt_dy =  0;       break;
        case EYE_LOOK_RIGHT:  g.tgt_dx =  LOOK_DX; g.tgt_dy =  0;       break;
        case EYE_LOOK_UP:     g.tgt_dx =  0;        g.tgt_dy = -LOOK_DY; break;
        case EYE_LOOK_DOWN:   g.tgt_dx =  0;        g.tgt_dy =  LOOK_DY; break;
        default:              g.tgt_dx =  0;        g.tgt_dy =  0;       break;
    }
