...
static void fetch_weather(void)
{
    ...
    if (parse_weather(s_http_buf, &fresh)) {
        s_data = fresh;
        ESP_LOGI(TAG, "Weather: %s %.1f C (feels %.1f C) hum %d%%",
                 s_data.condition, s_data.temp_c,
                 s_data.feels_like_c, s_data.humidity);

        /* Push to display — safe across tasks via lv_async_call */
...
void weather_init(void)
{
    ...
    ESP_LOGI(TAG, "Weather task started (city: %s, interval: %ds)",
             WEATHER_CITY, WEATHER_UPDATE_INTERVAL_S);
}

bool weather_get(weather_data_t *out)
{
    if (!out || !s_data.valid) return false;
    *out = s_data;
    return true;
}